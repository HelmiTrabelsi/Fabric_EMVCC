...
import (
	...
	"github.com/hyperledger/fabric/core/ledger/emvcc"
...
var _ = Describe("Endorser", func() {
	...
	BeforeEach(func() {
		...
		SimResults := &rwset.TxReadWriteSet{
			DataModel: rwset.TxReadWriteSet_KV,
			NsRwset: []*rwset.NsReadWriteSet{

				{
					Namespace: "myCC456",
					Rwset:     []byte{26, 79, 10, 12, 67, 108, 105, 101, 110, 116, 48, 95, 67, 65, 82, 56, 26, 63, 123, 34, 109, 97, 107, 101, 34, 58, 34, 86, 111, 108, 107, 115, 119, 97, 103, 101, 110, 34, 44, 34, 109, 111, 100, 101, 108, 34, 58, 34, 83, 34, 44, 34, 99, 111, 108, 111, 117, 114, 34, 58, 34, 98, 108, 117, 101, 34, 44, 34, 111, 119, 110, 101, 114, 34, 58, 34, 77, 97, 120, 34, 125},
				},
			},
		}
		fakeTxSimulator = &fake.TxSimulator{}
		fakeTxSimulator.GetTxSimulationResultsReturns(
			&ledger.TxSimulationResults{
				PubSimulationResults: SimResults,
	...
	JustBeforeEach(func() {
		...
		e.EndorserCache = emvcc.CreateCach()
		MVCCValidator := emvcc.GetMVCCValidatorFromCach(e.EndorserCache)
		// MVCCValidator := emvcc.NewMVCCValidator()
	...
	It("When Validating EMVCC succseed", func() {
		fmt.Println("When Validating EMVCC succseed")
		// fmt.Println(fakeTxSimulator.GetTxSimulationResults())
	...
	Context("when validating EMVCC fails", func() {
		BeforeEach(func() {
			...
			SimResults := &rwset.TxReadWriteSet{
				...
				NsRwset: []*rwset.NsReadWriteSet{
					{
						Namespace: "myCC12345",
						Rwset:     []byte{18, 74, 10, 3, 110, 115, 49, 18, 67, 10, 11, 10, 3, 100, 101, 102, 18, 4, 8, 1, 16, 2, 10, 12, 10, 4, 107, 101, 121, 49, 18, 4, 8, 1, 16, 1, 26, 18, 10, 4, 107, 101, 121, 53, 26, 10, 118, 97, 108, 117, 101, 49, 95, 110, 101, 119, 26, 18, 10, 4, 107, 101, 121, 54, 26, 10, 118, 97, 108, 117, 101, 50, 95, 110, 101, 119},
					},
		...
		It("When Validating EMVCC fails", func() {
			fmt.Println("when validating EMVCC fails")
			// fmt.Println(fakeTxSimulator.GetTxSimulationResults())
			// AddNewMVCCValidator
			MVCCValidator := emvcc.GetMVCCValidatorFromCach(e.EndorserCache)
			...
			MVCCValidator.AddKey("TX1", "key5")
			proposalResponse, err := e.ProcessProposal(context.TODO(), signedProposal)
			fmt.Println(proposalResponse)
			fmt.Println(err)
			Expect(err).NotTo(HaveOccurred())
			/*Expect(proto.Equal(proposalResponse.Response, &pb.Response{
				Status:  500,
				Message: "error in simulation: error while doing EMVCC validation: MVCC_READ_CONFLICT",
			})).To(BeTrue())*/
		})
	})
		...
		BeforeEach(func() {
			...
			SimResults := &rwset.TxReadWriteSet{
				...
				NsRwset: []*rwset.NsReadWriteSet{
					{
						Namespace: "myCC123",
						Rwset:     []byte{18, 73, 10, 3, 110, 115, 49, 18, 66, 10, 11, 10, 3, 97, 98, 99, 18, 4, 8, 1, 16, 1, 10, 11, 10, 3, 100, 101, 102, 18, 4, 8, 1, 16, 2, 26, 18, 10, 4, 107, 101, 121, 53, 26, 10, 118, 97, 108, 117, 101, 49, 95, 110, 101, 119, 26, 18, 10, 4, 107, 101, 121, 54, 26, 10, 118, 97, 108, 117, 101, 50, 95, 110, 101, 119},
					},
				},
			}
			fakeTxSimulator.GetTxSimulationResultsReturns(
