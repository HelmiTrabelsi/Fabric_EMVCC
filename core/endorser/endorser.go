...
func (e *Endorser) SimulateProposal(txParams *ccprovider.TransactionParams, chaincodeName string, chaincodeInput *pb.ChaincodeInput) (*pb.Response, []byte, *pb.ChaincodeEvent, error) {
	...
	/*if err != nil {
		logger.Errorf("failed to find MVCCValidator: %+v", err)
		return nil, nil, nil, err
	}*/
	fmt.Print("Before RW operation: ")
	...
	NSRWsets := simResult.PubSimulationResults.NsRwset
	fmt.Print("NSRWsets: ")
	fmt.Println(NSRWsets)
	/*if len(NSRWsets) > 1 {
		for i := 1; i <= len(NSRWsets. ); i++ {
			fmt.Print("NSRWsets[i]")
			fmt.Println(NSRWsets[i])
			pubRWSet.FromProtoBytes(NSRWsets[i].GetRwset())
			fmt.Print("pubRWSet")
			fmt.Println(pubRWSet)
			TxValidationCode, err := emvcc.TxValidation(MVCCValidator, pubRWSet)
			if err != nil {
				fmt.Println("An error occur while Getting TxValidationCode")
				return nil, nil, nil, err
			}
			if TxValidationCode != pb.TxValidationCode_VALID {
				fmt.Println("EMVCC error occur")
				err1 := errors.New("error while doing EMVCC validation: " + TxValidationCode.String())
	...
	*/
	for _, nsrwset := range NSRWsets {
		...
		fmt.Print("kvRwset:")
		fmt.Println(kvRwset)
		reads := kvRwset.GetReads()
		fmt.Print("reads:")
		fmt.Println(reads)

		writes := kvRwset.GetWrites()
		fmt.Print("Writes:")
		fmt.Println(writes)

		TxValidationCode, err := emvcc.TxValidation(MVCCValidator, reads)
		if err != nil {
			fmt.Println("An error occur while Getting TxValidationCode")
			return nil, nil, nil, err
		}
		...
		// write wset to the MVCCValidator
		MVCCValidator.AddToMVCCValidator(writes, txParams.TxID)
	}
	//fmt.Print("txParams.TxID :")
	//fmt.Println(txParams.TxID)
	fmt.Print("After RW operation: ")
	fmt.Println(MVCCValidator)
...
func (e *Endorser) GetEndoserCache() *cache.Cache {
	return e.EndorserCache
}
