...
func (v *validator) validateAndPrepareBatch(blk *block, doMVCCValidation bool) (*publicAndHashUpdates, error) {
	...
	// v.mvcccache = emvcc.CreateCach()
	MVCCValidator := emvcc.GetMVCCValidatorFromCach(v.mvcccache)
	...
	for _, tx := range blk.txs {
		...
		if validationCode == peer.TxValidationCode_VALID {
			...
			MVCCValidator.RemoveFromMVCCValidator(tx.id)
		} else {
