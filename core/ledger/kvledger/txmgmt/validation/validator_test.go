...
func TestValidatorBulkLoadingOfCache(t *testing.T) {
	...
	cache := emvcc.CreateCach()

	testValidator := &validator{db: db, hashFunc: testHashFunc, mvcccache: cache}
...
func TestValidator(t *testing.T) {
	...
	// fmt.Println("init MVCC validator")

	testValidator := &validator{db: db, hashFunc: testHashFunc, mvcccache: cache}
	MVCCValidator := emvcc.GetMVCCValidatorFromCach(testValidator.mvcccache)
	fmt.Print("Init MVCCValidator: ")
	fmt.Println(MVCCValidator)
	MVCCValidator.AddKey("txid-0", "key1")
	MVCCValidator.AddKey("TX2", "key2")
	MVCCValidator.AddKey("TX1", "key3")
	MVCCValidator.AddKey("TX1", "key5")
	// emvcc.SetMVCCValidatorInCach(testValidator.mvcccache, MVCCValidator)
	fmt.Print("Before Validation: ")
	fmt.Println(MVCCValidator)
