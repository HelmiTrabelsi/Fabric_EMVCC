...
func serve(args []string) error {
	...
	mvcccache := emvcc.CreateCach()
	MVCCValidator := emvcc.NewMVCCValidator()
	a := emvcc.SetMVCCValidatorInCach(mvcccache, MVCCValidator)
	if !a {
		return errors.WithMessage(err, "failed to init MVCCValidator")
	}
